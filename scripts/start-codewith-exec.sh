...
cd "${remote_codex_rs}"
cargo build -p codex-cli --bin codewith

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/codewith exec-server --listen ws://127.0.0.1:0 \
...
echo "Start codewith via: "
printf '  CODEX_EXEC_SERVER_URL=ws://127.0.0.1:%s codewith -C %q\n' \
