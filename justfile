...
codewith *args:
    cargo run --bin codewith -- {args}

# `codewith exec`
exec *args:
    cargo run --bin codewith -- exec {args}
...
app-server-test-client *args:
    ...
    cargo run -p codex-app-server-test-client -- --codex-bin ./target/debug/codewith {args}
...
[windows]
bazel-codex *args:
    bazel run //codex-rs/cli:codewith --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)
