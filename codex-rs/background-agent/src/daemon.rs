...
const DAEMON_START_HEALTH_CHECK_DELAY: Duration = Duration::from_millis(500);
...
impl BackgroundAgentDaemon {
    ...
    pub async fn start(&self) -> Result<BackgroundAgentDaemonOutput> {
        ...
        sleep(DAEMON_START_HEALTH_CHECK_DELAY).await;
        match self.controller.status(&record.handle).await? {
            WorkerProcessStatus::Running => {}
            WorkerProcessStatus::Missing | WorkerProcessStatus::StalePidRecord => {
                remove_pid_file(&self.paths.pid_file()).await?;
                bail!(
                    "background-agent daemon exited during startup; see stderr log {}",
                    self.paths.stderr_log_path().display()
                );
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn daemon_start_fails_when_child_exits_immediately() -> Result<()> {
        let temp_dir = TempDir::new().expect("temp dir");
        let daemon = BackgroundAgentDaemon::new(BackgroundAgentDaemonPaths::new(
            "/bin/false",
            temp_dir.path(),
        ...
        let err = daemon
            .start()
            .await
            .expect_err("daemon should fail when child exits immediately");

        assert!(err.to_string().contains("exited during startup"));
        assert!(read_pid_record(&daemon.paths.pid_file()).await?.is_none());
        Ok(())
    }
