...
`codex-network-proxy` is Codewith's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from Codewith's merged `config.toml` (via `codex-core` config loading).
...
# bundle env vars pointing at immutable public files under $CODEWITH_HOME/proxy/ so common HTTPS
...
When `codex-network-proxy` is embedded in managed Codewith runtime, policy decisions emit structured
