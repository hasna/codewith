...
#[command(
    ...
    about = "Run one Codewith turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    turn_output?;
    shutdown_result.context("shut down Codewith thread")?;

    Ok(())
}

fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    let codex_home = find_codex_home().context("find Codewith home")?;
    ...
    let mut config = Config {
        ...
        experimental_smart_suggest: Default::default(),
...
async fn run_turn(thread: &CodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread.next_event().await.context("read Codewith event")?;
