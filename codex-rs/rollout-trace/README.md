...
> **Privacy:** Rollout tracing is not telemetry. Codewith does **not** upload or
...
during a Codewith session. It records raw runtime evidence into a local bundle on
...
Hot-path Codewith code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Codewith got
there.
...
Codewith session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `codewith debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Codewith rollout/session being observed. Keeping those separate lets us reason about
