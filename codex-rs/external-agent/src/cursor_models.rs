//! Cursor Composer model catalog and live discovery.
//!
//! This module is the single model seam shared across the Cursor integration.
//! It merges two concerns that were developed in parallel:
//!
//! * **Live discovery** ([`discover_cursor_composer_models`]) queries the Cursor
//!   CLI/SDK (the equivalent of `Cursor.models.list()`) for the Composer models
//!   available to the current login, parses the result, caches it for the
//!   lifetime of the process, and gracefully falls back to
//!   [`CURSOR_DEFAULT_COMPOSER_MODEL_ID`] (`composer-2.5`) whenever discovery is
//!   impossible (the CLI is missing, offline, unauthenticated, times out, or
//!   returns output Codewith cannot parse). This is what the app-server RPC and
//!   the Cursor ACP adapter's advertised `_meta` default consume.
//! * **The offline catalog** ([`CursorComposerModelCatalog`],
//!   [`cursor_composer_seed_models`], [`resolve_cursor_composer_model`]) pins a
//!   minimal seed catalog and provides synchronous model selection for the local
//!   `@cursor/sdk` and cloud (`bc-`) backends, which resolve a requested model id
//!   without paying for an async discovery round-trip.
//!
//! [`CursorComposerModel`] is the shared model type for the whole external-agent
//! crate: live discovery, the static fallback, the seed catalog, and the
//! app-server RPC all speak in terms of it.
...
///
/// Alias of [`CURSOR_DEFAULT_COMPOSER_MODEL_ID`] so the runtime-side default and
/// the discovery default share a single source of truth.
pub const CURSOR_COMPOSER_DEFAULT_MODEL: &str = CURSOR_DEFAULT_COMPOSER_MODEL_ID;
...
/// the static fallback, the seed catalog, and the app-server RPC all speak in
/// terms of it.
...
/// [`CursorComposerModelCatalog::resolve`] cannot fail. It backs the synchronous
/// [`resolve_cursor_composer_model`] selection used by the local `@cursor/sdk`
/// and cloud backends.
...
impl CursorComposerModelCatalog {
    /// The offline seed catalog used for synchronous selection.
    pub fn seed() -> Self {
    ...
    pub fn from_models(models: Vec<CursorComposerModel>) -> Self {
        ...
        if !deduped.iter().any(|model| model.is_default) {
    ...
    pub fn from_discovery_json(value: &JsonValue) -> Option<Self> {
        ...
        for entry in array {
            ...
            let is_default = entry
                ...
                .unwrap_or(false);
            models.push(CursorComposerModel::new(id, display_name, is_default));
        }
        if models.is_empty() {
            return None;
        }
...
/// Seed model list used for synchronous selection when a live catalog is not
...
mod tests {
    ...
    #[test]
    fn catalog_discovery_parses_bare_array_and_promotes_default() {
    ...
    #[test]
    fn catalog_discovery_parses_wrapped_object_and_honors_explicit_default() {
        let value = serde_json::json!({
            "models": [
    ...
    #[test]
    fn catalog_discovery_dedupes_and_keeps_single_default() {
        let value = serde_json::json!([
            {"id": "a", "default": true},
            {"id": "a", "default": true},
            {"id": "b", "default": true},
    ...
    #[test]
    fn catalog_discovery_returns_none_without_models() {
