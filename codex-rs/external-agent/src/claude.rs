...
use crate::claude_auth::AgentSdkAuthEnv;
use crate::claude_auth::add_agent_sdk_auth_env;
use crate::claude_auth::agent_sdk_auth_env;
...
impl ClaudeCodeHarness {
    ...
    async fn run_sandboxed_launch(
        ...
        let mut process = ClaudeCodeProcess::spawn(launch)?;
    ...
    fn runtime_missing_provider_auth_readiness(&self, detail: String) -> ExternalAgentReadiness {
        ExternalAgentReadiness {
            runtime: self.id(),
            status: ExternalAgentReadinessStatus::MissingAuth,
            display_name: self.descriptor.display_name.to_string(),
...
impl ClaudeCodeProcess {
    fn spawn(launch: ExternalAgentSandboxedLaunchSpec) -> Result<Self, ExternalAgentError> {
...
"#,
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn readiness_accepts_provider_auth_env_without_local_login() {
        let (_temp_dir, mut env) = fake_claude_env(
            r#"#!/bin/sh
if [ "$1" = "auth" ] && [ "$2" = "status" ]; then
  exit 1
...
"#,
        ...
        env.insert("AWS_PROFILE".to_string(), "dev".to_string());
...
"#,
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn readiness_reports_missing_auth_for_provider_selector_without_route_evidence() {
        let (_temp_dir, mut env) = fake_claude_env(
            r#"#!/bin/sh
if [ "$1" = "auth" ] && [ "$2" = "status" ]; then
  exit 0
...
"#,
    ...
    #[test]
    fn launch_env_preserves_stable_config_and_provider_auth_only() {
        let harness = claude_code_harness().expect("claude harness");
        ...
        let spec = harness.launch_spec("/repo", "/usr/bin/claude", &source, "inspect this repo");

        assert_eq!(spec.args, claude_code_args("inspect this repo"));
