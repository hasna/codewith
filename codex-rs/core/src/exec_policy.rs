    ...
    if dangerous_command_match.is_some() || windows_managed_fs_restrictions_without_sandbox_backend
    {
        return match approval_policy {
            AskForApproval::Never => {
                // FORK DEVIATION from upstream #33464: upstream unconditionally
                // forbids dangerous commands under `Never`. We preserve the
                // explicit-disable escape hatch because `PermissionProfile::External`
                // is owned by our infinity-agent / external-agent architecture,
                // where containment is enforced by the external sandbox. Forbidding
                // here would regress those managed flows.
...
/// - Note: we only use this amendment when the command fails to run in sandbox and Codewith prompts the user to run outside the sandbox
