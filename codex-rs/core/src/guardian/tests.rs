...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Codewith agent has requested the following action:\n"));
...
async fn build_guardian_prompt_delta_mode_preserves_original_numbering() -> anyhow::Result<()> {
    ...
    assert!(text.contains("The Codewith agent has requested the following next action:\n"));
...
#[test]
fn parse_guardian_assessment_treats_nullable_allow_details_as_low_risk() {
    let parsed = parse_guardian_assessment(Some(
...
#[test]
fn guardian_output_schema_is_strict_response_format_compatible() {
