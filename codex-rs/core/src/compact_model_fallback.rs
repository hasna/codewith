...
mod tests {
    ...
    #[test]
    fn retries_model_specific_failures() {
        // A resumed thread whose model was renamed surfaces as an invalid request; retry it with
        // the currently selected model rather than failing the turn.
        assert!(should_retry_with_current_model(&CodexErr::InvalidRequest(
            "model not found".to_string()
        )));
        assert!(should_retry_with_current_model(
            &CodexErr::ContextWindowExceeded
        ));
        assert!(should_retry_with_current_model(&CodexErr::ServerOverloaded));
        assert!(should_retry_with_current_model(
            &CodexErr::InternalServerError
        ));
    }
    ...
    fn does_not_retry_unrelated_failures() {
        assert!(!should_retry_with_current_model(&CodexErr::TurnAborted));
        assert!(!should_retry_with_current_model(&CodexErr::Interrupted));
        assert!(!should_retry_with_current_model(&CodexErr::Stream(
            "boom".to_string(),
            None
        )));
    }
