...
impl MetadataReadCounts {
    fn snapshot(&self) -> MetadataReadSnapshot {
        MetadataReadSnapshot {
            ...
            supports_parallel_tool_calls: self.supports_parallel_tool_calls.load(Ordering::SeqCst),
        }
    }
}
    ...
    fn search_info(&self) -> Option<codex_tools::ToolSearchInfo> {
        ...
        Some(codex_tools::ToolSearchInfo {
            entry: codex_tools::ToolSearchEntry {
                ...
                output: codex_tools::LoadableToolSpec::Function(codex_tools::ResponsesApiTool {
                    name: "extension_echo".to_string(),
...
struct MismatchedProjectionExtensionExecutor;

#[async_trait::async_trait]
impl codex_extension_api::ToolExecutor<codex_tools::ToolCall>
    for MismatchedProjectionExtensionExecutor
{
    fn tool_name(&self) -> codex_tools::ToolName {
        codex_tools::ToolName::plain("extension_echo")
    }
    ...
    fn search_info(&self) -> Option<codex_tools::ToolSearchInfo> {
        Some(codex_tools::ToolSearchInfo {
            entry: codex_tools::ToolSearchEntry {
                search_text: "untrusted argument contract".to_string(),
                output: codex_tools::LoadableToolSpec::Function(codex_tools::ResponsesApiTool {
                    name: "extension_echo".to_string(),
                    description: "Projection with a different contract.".to_string(),
                    strict: false,
                    parameters: codex_tools::JsonSchema::default(),
                    output_schema: None,
                    defer_loading: Some(true),
    ...
    async fn handle(
        ...
        panic!("projection validation must not execute extension tools")
    }
}

#[test]
fn deferred_search_info_cannot_replace_the_admitted_argument_contract() {
    let handler = ExtensionToolAdapter::new(Arc::new(MismatchedProjectionExtensionExecutor));
    ...
    assert!(matches!(
        ...
        codex_tools::ToolSpec::Function(tool)
            if tool.strict
                && tool.parameters.properties.as_ref().is_some_and(|properties| {
...
fn deferred_extension_without_search_info_stays_undiscoverable() {
    let handler = ExtensionToolAdapter::new(Arc::new(DeferredWithoutSearchInfoExtensionExecutor));
