...
pub(crate) fn strip_managed_proxy_env(env: &mut HashMap<String, String>) {
    ...
    // Only macOS injects a Codewith-owned SSH wrapper for the managed SOCKS proxy.
    #[cfg(target_os = "macos")]
    if env
...
///
/// `runtime_path_prepends` contains Codewith-owned PATH entries already applied to
