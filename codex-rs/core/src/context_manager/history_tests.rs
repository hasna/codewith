...
fn assert_oldest_prefix_trim_preserves_last_pair(pair: Vec<ResponseItem>) {
    let mut history = create_history_with_items(
        std::iter::once(user_input_text_msg("old user turn"))
            .chain(pair.clone())
            .collect(),
    );

    assert_eq!(history.remove_oldest_items_preserving_pairs(1), 1);
    assert_eq!(history.raw_items(), pair);
    assert_eq!(history.remove_oldest_items_preserving_pairs(1), 0);
    assert_eq!(history.raw_items(), pair);
}

#[test]
fn remove_oldest_items_preserving_pairs_keeps_last_function_call_unit() {
    let pair = vec![
        ResponseItem::FunctionCall {
            id: None,
            name: "do_it".to_string(),
    ...
    assert_oldest_prefix_trim_preserves_last_pair(pair);
}

#[test]
fn remove_oldest_items_preserving_pairs_keeps_last_tool_search_unit() {
    let pair = vec![
        ResponseItem::ToolSearchCall {
            id: None,
            call_id: Some("search-last".to_string()),
            status: Some("completed".to_string()),
            execution: "client".to_string(),
            arguments: serde_json::json!({"query": "last query"}),
        },
...
#[test]
fn remove_oldest_items_preserving_pairs_keeps_last_local_shell_unit() {
    let pair = vec![
        ResponseItem::LocalShellCall {
            id: None,
            call_id: Some("shell-last".to_string()),
...
#[test]
fn remove_oldest_items_preserving_pairs_keeps_last_custom_tool_unit() {
    let pair = vec![
        ResponseItem::CustomToolCall {
            id: None,
            status: Some("completed".to_string()),
            call_id: "custom-last".to_string(),
...
#[test]
fn remove_oldest_items_preserving_pairs_keeps_interleaved_pairs_atomic() {
    let surviving_item = user_input_text_msg("surviving logical unit");
    let items = vec![
        function_call("call-a"),
        function_call("call-b"),
        function_call_output("call-a", "a"),
        function_call_output("call-b", "b"),
        surviving_item.clone(),
    ];
    let mut history = create_history_with_items(items);

    assert_eq!(history.remove_oldest_items_preserving_pairs(1), 4);
    assert_eq!(history.raw_items(), &[surviving_item]);
}

#[test]
fn remove_oldest_items_preserving_pairs_groups_duplicate_call_ids() {
    let surviving_item = user_input_text_msg("surviving logical unit");
    let items = vec![
        function_call("duplicate-call"),
        function_call("duplicate-call"),
        function_call_output("duplicate-call", "first"),
        function_call_output("duplicate-call", "second"),
        surviving_item.clone(),
    ];
    let mut history = create_history_with_items(items);
...
#[test]
fn remove_oldest_items_preserving_pairs_does_not_leave_only_an_orphan_output() {
    let orphan_outputs = [
        ResponseItem::FunctionCallOutput {
        ...
        ResponseItem::CustomToolCallOutput {
            call_id: "orphan-custom-call".to_string(),
            name: Some("custom_tool".to_string()),
            output: FunctionCallOutputPayload::from_text("orphan output".to_string()),
        },
    ];
    ...
    for orphan_output in orphan_outputs {
        let items = vec![user_input_text_msg("last logical unit"), orphan_output];
        ...
        assert!(
            history
                .raw_items()
                .iter()
                .any(|item| normalize::required_call_for_output(item).is_none())
        );
    }
}
...
fn drop_last_n_user_turns_ignores_session_prefix_user_messages() {
    let items = vec![
        ...
        user_input_text_msg(
            "# CODEWITH.md instructions for test_directory\n\n<INSTRUCTIONS>\ntest_text\n</INSTRUCTIONS>",
        ),
        user_input_text_msg(
