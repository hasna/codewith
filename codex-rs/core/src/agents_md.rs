...
impl<'a> AgentsMdManager<'a> {
    ...
    async fn read_agents_md(
        ...
        let docs = self.discover_agents_md(fs, startup_warnings).await?;
        if docs.is_empty() {
            return Ok(None);
        }
    ...
    async fn agents_md_paths(
        ...
        Ok(self
            .discover_agents_md(fs, &mut startup_warnings)
            .await?
            .into_iter()
            .map(|doc| doc.path)
            .collect())
    ...
    Box::pin(async move {
        ...
        if metadata.is_symlink {
            if is_import {
                ...
                warn_skipped_symlink(&path, ctx.source_label, ctx.startup_warnings);
            }
            return Ok(Vec::new());
        }
            ...
            .await
        {
            ...
            Err(err) if is_file_changed_during_open(&err) => {
                warn_file_changed_during_open(&path, ctx.source_label, ctx.startup_warnings);
                return Ok(Vec::new());
            }
            Err(err) if is_symlink_safe_read_unsupported(&err) => {
                warn_symlink_safe_read_unsupported(&path, ctx.source_label, ctx.startup_warnings);
                return Ok(Vec::new());
            }
            Err(err) => return Err(err),
        };
