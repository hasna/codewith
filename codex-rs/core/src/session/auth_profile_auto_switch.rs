...
impl AuthProfileAutoSwitchTurnState {
    pub(crate) async fn next_profile_for_usage_limit(
        ...
        if !usage_limit_matches_config(&auto_switch_config, err) {
            return None;
        }
...
fn usage_limit_matches_config(
    ...
    let snapshot = err.rate_limits.as_deref();
...
mod tests {
    ...
    fn usage_error(rate_limits: Option<RateLimitSnapshot>) -> UsageLimitReachedError {
    ...
    fn model_snapshot(window_minutes: i64) -> RateLimitSnapshot {
        RateLimitSnapshot {
            limit_id: Some("gpt-5.5".to_string()),
            limit_name: Some("gpt-5.5".to_string()),
            primary: Some(RateLimitWindow {
                used_percent: 100.0,
                window_minutes: Some(window_minutes),
                resets_at: Some(123),
    ...
    fn usage_limit_matching_respects_enabled_windows() {
        ...
        assert!(usage_limit_matches_config(
            &config,
            &usage_error(Some(codex_snapshot(FIVE_HOUR_WINDOW_MINUTES)))
        ));
        ...
        assert!(usage_limit_matches_config(
            &config,
            &usage_error(Some(codex_snapshot(WEEKLY_WINDOW_MINUTES)))
        ));
    ...
    #[test]
    fn usage_limit_matching_accepts_live_incomplete_or_model_limit_metadata() {
        let config = AuthProfileAutoSwitchConfig {
            enabled: true,
            ..Default::default()
        };

        assert!(usage_limit_matches_config(&config, &usage_error(None)));
        assert!(usage_limit_matches_config(
            &config,
            &usage_error(Some(model_snapshot(FIVE_HOUR_WINDOW_MINUTES)))
        ));
    }
}
