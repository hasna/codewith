...
fn map_rollout_io_error(io_err: &std::io::Error, codex_home: &Path) -> Option<CodexErr> {
    ...
    let hint = match io_err.kind() {
        ErrorKind::PermissionDenied => format!(
            "Codewith cannot access session files at {} (permission denied). If sessions were created using sudo, fix ownership: sudo chown -R $(whoami) {}",
        ...
        ErrorKind::AlreadyExists => format!(
            "Session storage path {} is blocked by an existing file. Remove or rename it so Codewith can create sessions.",
            sessions_dir.display()
        ),
