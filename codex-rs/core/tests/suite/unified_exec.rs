...
async fn write_stdin_calls_run_in_parallel_across_sessions() -> Result<()> {
    ...
    let start_args = serde_json::to_string(&serde_json::json!({
        "cmd": "bash --noprofile --norc",
        "tty": true,
        "yield_time_ms": 250,
    }))?;
    let cross_session_a = serde_json::to_string(&serde_json::json!({
        "session_id": 1000,
        "chars": ": > .write-stdin-a; while [ ! -e .write-stdin-b ]; do sleep 0.01; done; printf 'alpha-%s\\n' ready; exit\n",
        "yield_time_ms": 5_000,
    }))?;
    let cross_session_b = serde_json::to_string(&serde_json::json!({
        "session_id": 1001,
