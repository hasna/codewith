...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    // Configure Codewith to resume from our file
    ...
    let pos_user_instructions = messages
        ...
        .position(|(role, text)| {
            role == "user"
                ...
                && (text.starts_with("# CODEWITH.md instructions for "))
        })
...
async fn includes_user_instructions_message_in_request() {
    ...
    assert!(
        user_context_texts
            .iter()
            .any(|text| text.starts_with("# CODEWITH.md instructions for ")),
...
async fn model_without_summary_parameter_support_omits_configured_summary() -> anyhow::Result<()> {
    ...
    let request_body = resp_mock.single_request().body_json();
    // A model that does not accept the summary parameter still sends reasoning
    // effort and encrypted content, but omits `reasoning.summary`.
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Codewith sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
