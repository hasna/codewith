...
`codex-app-server-daemon` backs the machine-readable `codewith app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Codewith instances launched over SSH, including fresh developer
...
```sh
codewith app-server daemon start
codewith app-server daemon restart
codewith app-server daemon enable-remote-control
codewith app-server daemon disable-remote-control
codewith app-server daemon stop
codewith app-server daemon version
codewith app-server daemon bootstrap --remote-control
```
...
$HOME/.codewith/packages/standalone/current/codewith app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `CODEWITH_HOME/app-server-daemon/`, starts app-server as a
...
Top-level `codewith remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `CODEWITH_HOME`, so a concurrent
...
The daemon stores its local state under `CODEWITH_HOME/app-server-daemon/`:
