...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let codex_home = root.path().join(".codewith");
    ...
    let expected = vec![
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                codex_home.join("CODEWITH.md").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".codewith").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".codewith").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
"#,
    ...
    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".codewith").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
...
Research with Codewith carefully."""
"#,
    )
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    ...
    assert_eq!(
        fs::read_to_string(codex_home.join("CODEWITH.md")).expect("read agents"),
        "Codewith guidance"
    );
...
"#,
    ...
    assert_eq!(
        ...
        "Use Codewith and Codewith utilities."
    );
}
...
async fn import_repo_hooks_preserves_disabled_codex_hooks_feature() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create external agent dir");
    fs::create_dir_all(repo_root.join(".codewith")).expect("create codex dir");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
...
async fn import_repo_mcp_uses_home_settings_toggles_when_repo_settings_missing() {
    ...
    service_for_paths(external_agent_home, root.path().join(".codewith"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
...
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_codex() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".codewith")).expect("create repo codex dir");
    fs::create_dir_all(&codex_home).expect("create codex home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
