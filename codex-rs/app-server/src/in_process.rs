...
mod tests {
    ...
    fn run_in_process_test_with_stack<F>(name: &str, future: F) -> IoResult<()>
    where
        ...
        let handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .expect("in-process test runtime should build")
                    .block_on(future)
            })?;

        handle
            .join()
            .map_err(|_| IoError::other(format!("{name} thread panicked")))?
    }
    ...
    #[test]
    fn in_process_start_uses_requested_session_source_for_thread_start() -> IoResult<()> {
        run_in_process_test_with_stack(
    ...
    #[test]
    fn in_process_start_clamps_zero_channel_capacity() -> IoResult<()> {
        run_in_process_test_with_stack("in_process_start_clamps_zero_channel_capacity", async {
            let client =
