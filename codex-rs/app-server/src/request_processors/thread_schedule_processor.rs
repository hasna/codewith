...
const MAX_SCHEDULE_LIMIT: usize = 50;
        ...
        let claim = state_db
            ...
            .ok_or_else(|| {
                invalid_request(format!(
                    "schedule is not active or is already running: {}",
                    schedule_id
                ))
            })?;
...
fn validate_schedule_prompt(prompt: &str) -> Result<String, JSONRPCErrorError> {
...
fn normalize_timezone(timezone: Option<String>) -> Result<String, JSONRPCErrorError> {
...
fn normalize_timezone_value(timezone: String) -> Result<String, JSONRPCErrorError> {
...
fn normalize_list_limit(limit: Option<u32>) -> Result<usize, JSONRPCErrorError> {
...
fn decode_cursor(cursor: Option<&str>) -> Result<usize, JSONRPCErrorError> {
...
fn api_thread_schedule_status_to_state(
...
fn api_thread_schedule_spec_to_state(
...
pub(super) fn thread_schedule_spec_from_state(
