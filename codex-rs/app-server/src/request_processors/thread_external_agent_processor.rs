...
impl ThreadRequestProcessor {
    ...
    async fn thread_external_agent_start_inner(
        ...
        let sandbox_config = ExternalAgentSandboxConfig {
            permission_profile: external_agent_permission_profile(),
...
fn external_agent_use_legacy_landlock() -> bool {
    cfg!(target_os = "linux")
}
...
mod tests {
    ...
    #[test]
    fn external_agent_uses_landlock_backend_on_linux() {
        assert_eq!(
            external_agent_use_legacy_landlock(),
            cfg!(target_os = "linux")
        );
    }
}
