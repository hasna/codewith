...
use codex_background_agent::BackgroundAgentEventForSupervisorParams;
use codex_background_agent::BackgroundAgentExecutionHandleParams;
use codex_background_agent::BackgroundAgentExecutionSnapshotForSupervisorParams;
...
use codex_background_agent::BackgroundAgentStatusSnapshotForSupervisorParams;
...
use codex_background_agent::BackgroundAgentWorkerWriteScope;
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn run_background_agent_worker_once(
        ...
        match &result {
            ...
            Err(err) => {
                mark_background_agent_worker_failed(
                    &context,
                    run.id.as_str(),
...
async fn reconcile_background_agents(
    ...
    for run in runs {
        ...
        let expected_generation = run.generation.saturating_add(1);
        context.task_tracker.spawn(async move {
            ...
            if let Err(err) = run_background_agent_worker(worker_context.clone(), run, token).await
            {
                ...
                if let Err(mark_err) = mark_background_agent_worker_failed(
                    &worker_context,
                    run_id.as_str(),
                    expected_generation,
                    &err,
...
async fn run_background_agent_worker(
    ...
    let snapshot =
        retry_transient_sqlite_busy("create background agent execution snapshot", || {
            context.state_db.create_execution_snapshot_for_supervisor(
...
async fn upsert_interaction_status_snapshot(
    ...
    let snapshot =
        retry_transient_sqlite_busy("upsert background agent waiting status snapshot", || {
            context.state_db.upsert_status_snapshot_for_supervisor(
                BackgroundAgentStatusSnapshotForSupervisorParams {
                    supervisor_id: context.supervisor_id.as_str(),
                    generation,
                    snapshot: BackgroundAgentStatusSnapshotParams {
                        run_id: run_id.to_string(),
...
async fn append_background_agent_event_with_retry(
    ...
    let write_scope = if allow_terminal_current {
        BackgroundAgentWorkerWriteScope::CurrentOrTerminal
    } else {
        BackgroundAgentWorkerWriteScope::Active
    };
    let event = retry_transient_sqlite_busy("append background agent event", || {
        context
            .state_db
            .append_event_for_supervisor(BackgroundAgentEventForSupervisorParams {
                run_id,
                supervisor_id: context.supervisor_id.as_str(),
                generation,
                write_scope,
