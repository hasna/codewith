...
//! the configured Codewith executable as a subprocess, accepts only valid JSON from
//! `codewith doctor --json`, derives a small set of Sentry tags, and otherwise
...
/// Runs `codewith doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Codewith, finish before the timeout, or parse JSON means the
