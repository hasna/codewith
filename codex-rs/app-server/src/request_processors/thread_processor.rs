...
pub(crate) struct ThreadRequestProcessor {
    ...
    pub(super) thread_mailbox_dispatcher_runtime: Option<ThreadMailboxDispatcherRuntime>,
...
impl ThreadRequestProcessor {
    ...
    async fn thread_resume_inner(
        ...
        let resume_result = if let Some(history) = history {
            ...
            // Reuse the history-bearing probe from `resume_running_thread` so cold
            // resume does not read and parse the entire rollout a second time.
