...
/// Cold resume probes the thread store for the stored thread (with history) to
/// decide whether a loaded thread can handle the request. That probe must be
/// reused for building the resumed session instead of reading — and parsing —
/// the full history a second time.
#[tokio::test]
async fn cold_thread_resume_reuses_history_probe() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(codex_home.path(), &server.uri(), &store_id)?;
    ...
    assert_eq!(
        ...
        "cold resume should read the stored thread history exactly once"
    );
