...
`codewith app-server` is the interface Codewith uses to power rich interfaces such as the [Codewith VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `codewith app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--stdio` or `--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$CODEWITH_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
The unix socket transport is intended for local app-server control-plane clients. `codewith app-server proxy`
opens exactly one raw stream connection to `$CODEWITH_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `codewith app-server generate-ts`, or a JSON Schema bundle via `codewith app-server generate-json-schema`. Each output is specific to the version of Codewith you used to run the command, so the generated artifacts are guaranteed to match that version.

```
codewith app-server generate-ts --out DIR
codewith app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Codewith:

- **Thread**: A conversation between a user and the Codewith agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's Codewith home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.

`initialize.params.capabilities` also supports per-connection notification opt-out via `optOutNotificationMethods`, which is a list of exact method names to suppress for that connection. Matching is exact (no wildcards/prefixes). Unknown method names are accepted and ignored.

Applications building on top of `codewith app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for compliance logs. If
you are developing a new Codewith integration that is intended for enterprise use, please contact Hasna to get it
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "codewith_vscode",
      "title": "Codewith VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `thread/workflow/update` — experimental; replace a saved workflow definition's YAML for the same materialized thread, recomputing sanitized metadata and returning `workflow: null` when no matching record exists.
- `thread/workflow/delete` — experimental; delete a saved workflow definition for the same materialized thread, returning `deleted: false` when missing and rejecting definitions that still have workflow runs.
...
- `thread/workflow/run/updated` — notification emitted when workflow run start, pause, resume, or cancel changes a run snapshot; includes the thread id and sanitized run state.
...
- `review/start` — kick off Codewith's automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
- `process/spawn` — experimental; spawn a standalone process without the Codewith sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
Start a fresh thread when you need a new Codewith conversation.
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current Codewith home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Use `thread/workflow/update` to replace the saved YAML for a definition in the same thread. The response returns the refreshed sanitized metadata, or `workflow: null` when the record is missing or belongs to another thread. Use `thread/workflow/delete` to remove a definition that has no existing workflow runs; a missing definition returns `deleted: false`.

```json
{ "method": "thread/workflow/update", "id": 36, "params": {
    "threadId": "thr_123",
    "workflowRecordId": "workflow_123",
    "yaml": "schema_version: \"workflow.codex.codewith/v0\"\nworkflow_id: \"wf_dentist_leads\"\ndisplay_name: \"Dentist Lead SaaS v2\"\n..."
} }
{ "id": 36, "result": { "workflow": { "workflowRecordId": "workflow_123", "displayName": "Dentist Lead SaaS v2" } } }

{ "method": "thread/workflow/delete", "id": 37, "params": {
    "threadId": "thr_123",
    "workflowRecordId": "workflow_123"
} }
{ "id": 37, "result": { "deleted": true } }
```
...
```json
{ "method": "thread/workflow/run/start", "id": 38, "params": {
    "threadId": "thr_123",
    "workflowRecordId": "workflow_123",
...
{ "method": "thread/workflow/run/list", "id": 39, "params": { "threadId": "thr_123", "limit": 20 } }
{ "id": 39, "result": { "data": [{ "runId": "run_123", "status": "pending" }], "nextCursor": null } }

{ "method": "thread/workflow/run/pause", "id": 40, "params": { "threadId": "thr_123", "runId": "run_123", "reason": "pause for review" } }
{ "id": 40, "result": { "run": { "run": { "runId": "run_123", "status": "paused" } } } }
{ "method": "thread/workflow/run/updated", "params": { "threadId": "thr_123", "run": { "runId": "run_123", "status": "paused" } } }

{ "method": "thread/workflow/run/resume", "id": 41, "params": { "threadId": "thr_123", "runId": "run_123" } }
{ "id": 41, "result": { "run": { "run": { "runId": "run_123", "status": "waiting" } } } }

{ "method": "thread/workflow/run/cancel", "id": 42, "params": { "threadId": "thr_123", "runId": "run_123", "reason": "stop this workflow" } }
{ "id": 42, "result": { "run": { "run": { "runId": "run_123", "status": "cancelRequested" } } } }
```
...
Set `promptSource` to `"inline"` to store and send `prompt` directly. Set it to `"default"` to resolve the prompt at execution time from `.codewith/loop.md` in the thread cwd, then `$CODEWITH_HOME/loop.md`; the stored `prompt` field is only the display label in that mode.
...
Turns attach user input (text or images) to a thread and trigger Codewith generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.codewith/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a Codewith session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use Codewith's default realtime backend prompt. Send `prompt: null` or
...
Use `review/start` to run Codewith’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Codewith will run).
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Codewith will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, Codewith generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the Codewith sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a Codewith session/thread-group id.
...
- `contextCompaction` — `{id}` emitted when Codewith compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when Codewith compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Codewith whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.codewith/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Codewith skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Codewith skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.codewith/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.codewith/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
Codewith supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Codewith owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; Codewith persists tokens to disk and refreshes them automatically.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Codewith can run without OpenAI credentials.
...
`codewith app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
codewith app-server generate-ts --out DIR --experimental
codewith app-server generate-json-schema --out DIR --experimental
```
