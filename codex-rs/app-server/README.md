...
- `model/safetyBuffering/updated` — `{ threadId, turnId, model, useCases, reasons, showBufferingUi, fasterModel, learnMoreLink }` when a response enters safety buffering. `fasterModel` and `learnMoreLink` are nullable. This notification is transient and is not persisted in rollout history.
