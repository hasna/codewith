...
3. Generate or update the personal marketplace entry when the plugin should appear in Codewith UI ordering:
...
- Treat plugin order in `plugins[]` as render order in Codewith. Append new entries unless a user explicitly asks to reorder the list.
...
- If Codewith would need approval to write the marketplace file, ask for that approval before
...
- If the user provided a non-default `--marketplace-path`, make sure that marketplace is installed
  before giving reinstall instructions. Use `codewith plugin marketplace add <path-to-marketplace-root>`
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short Codewith app handoff. Say `To view this in the Codewith app:` and write
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. Codewith derives both after
