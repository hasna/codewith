...
Set a stable path to the skill CLI (default `CODEWITH_HOME` is `~/.codewith`):

```
export CODEWITH_HOME="${CODEWITH_HOME:-$HOME/.codewith}"
export IMAGE_GEN="$CODEWITH_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$CODEWITH_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
