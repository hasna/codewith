...
#[tokio::test]
async fn workflow_manager_loaded_replaces_loading_view() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let thread_id = ThreadId::new();

    chat.show_thread_workflow_manager_loading(thread_id);
    chat.show_thread_workflow_manager(
        thread_id,
        ThreadWorkflowListResponse {
            data: vec![test_workflow("workflow-alpha", ThreadWorkflowStatus::Draft)],
    ...
    assert!(
        ...
        "loaded workflow manager should replace loading view instead of stacking above it"
    );
}

#[tokio::test]
async fn workflow_manager_redacts_sensitive_spec_id() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let thread_id = ThreadId::new();
    let mut workflow = test_workflow("workflow-alpha", ThreadWorkflowStatus::Draft);
    workflow.spec_workflow_id = "source_prompt secret workflow id".to_string();
    ...
    assert!(popup.contains("[redacted]"));
    assert!(!popup.contains("source_prompt secret workflow id"));
}
