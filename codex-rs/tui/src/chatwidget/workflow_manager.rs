...
const WORKFLOW_MANAGER_VIEW_ID: &str = "workflow-manager";

impl ChatWidget {
    pub(crate) fn show_thread_workflow_manager_loading(&mut self, thread_id: ThreadId) {
        self.replace_or_show_thread_workflow_manager(thread_workflow_manager_loading_params(
            thread_id,
        ));
    }
    ...
    pub(crate) fn show_thread_workflow_manager_error(
        ...
        self.replace_or_show_thread_workflow_manager(thread_workflow_manager_error_params(
            thread_id,
            action,
            &err.to_string(),
        ));
    ...
    pub(crate) fn show_thread_workflow_manager(
        ...
        self.replace_or_show_thread_workflow_manager(thread_workflow_manager_params(
            thread_id,
            workflows.data,
    ...
    fn replace_or_show_thread_workflow_manager(&mut self, params: SelectionViewParams) {
        if self.bottom_pane.active_view_id() == Some(WORKFLOW_MANAGER_VIEW_ID) {
            let replaced = self
                .bottom_pane
                .replace_selection_view_if_active(WORKFLOW_MANAGER_VIEW_ID, params);
...
fn thread_workflow_manager_loading_params(thread_id: ThreadId) -> SelectionViewParams {
    ...
    SelectionViewParams {
        view_id: Some(WORKFLOW_MANAGER_VIEW_ID),
...
fn workflow_selected_detail(workflow: &ThreadWorkflow) -> String {
    middle_dot(vec![
        sanitize_metadata_label(&workflow.spec_workflow_id),
