...
impl HistoryCell for UpdateAvailableHistoryCell {
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            "See https://github.com/hasna/codewith for installation options.".to_string()
        };
        vec![
            ...
            Line::from("https://github.com/hasna/codewith/releases/latest"),
        ]
    }
...
pub(crate) fn new_session_recap_event(summary: String) -> PlainHistoryCell {
    let lines: Vec<Line<'static>> =
        vec![vec!["• ".dim(), format!("Recap: {summary}").dark_gray()].into()];
    PlainHistoryCell { lines }
}
