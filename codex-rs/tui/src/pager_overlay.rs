...
mod tests {
    ...
    #[test]
    fn transcript_overlay_remeasures_dynamic_cell_on_later_renders() {
        // A cell reporting an unstable transcript height must be remeasured on every overlay
        // render so refreshed content (e.g. status output or newly available visualizations)
        // is never clipped by a stale cached height.
        let height_calls = Arc::new(AtomicUsize::new(0));
        let mut overlay = transcript_overlay(vec![Arc::new(HeightCountingCell {
            height_calls: height_calls.clone(),
            stable: false,
        ...
        overlay.render(area, &mut buf);
        let after_first = height_calls.load(Ordering::Relaxed);
        assert!(after_first >= 1);

        overlay.render(area, &mut buf);
        assert!(
            height_calls.load(Ordering::Relaxed) > after_first,
            "expected dynamic cell to be remeasured on the second render",
        );
    }
