...
fn create_tool_input_schema(schema: schemars::Schema, panic_message: &str) -> Arc<JsonObject> {
    ...
    remove_optional_null_types(&mut schema_value);
...
fn remove_optional_null_types(value: &mut serde_json::Value) {
    match value {
...
fn normalize_description(description: &mut serde_json::Value) {
    let serde_json::Value::String(description_text) = description else {
        return;
    };
...
fn remove_null_enum_variant(enum_value: &mut serde_json::Value) {
    let serde_json::Value::Array(values) = enum_value else {
        return;
    };
...
fn remove_null_type(type_value: &mut serde_json::Value) {
    let serde_json::Value::Array(types) = type_value else {
        return;
    };

    let non_null_types = types
        .iter()
        .filter(|value| !matches!(value, serde_json::Value::String(value) if value == "null"))
        .cloned()
        .collect::<Vec<_>>();
