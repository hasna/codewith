...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Codewith after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the Codewith user.
    #[arg(
        long = "current-user",
    ...
    /// CODEX_HOME for the Codewith user. Required with --user.
