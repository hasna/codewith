...
use codex_state::BackgroundAgentPendingInteraction;
...
pub(crate) enum AgentSubcommand {
    ...
    #[clap(visible_alias = "run", visible_alias = "spawn")]
    ...
    #[clap(visible_alias = "diag", visible_alias = "daemon")]
    Diagnostics,
}
...
pub(crate) async fn run_agent_command(
    ...
    let output = match cli.subcommand {
        ...
        AgentSubcommand::Delete(cmd) => {
            let (deleted, run) = delete_agent(state_db.as_ref(), cmd.agent_id.as_str()).await?;
        ...
        .create_background_agent_execution_snapshot(&BackgroundAgentExecutionSnapshotParams {
            ...
            payload_json: json!({
                ...
                "permissionProfile": runtime_context
                    .and_then(|context| context.permission_profile.as_deref()),
...
async fn stop_agent(
    ...
    if !background_agent_status_is_terminal(run.status) {
        ...
        if terminalize_immediately {
            cancel_active_pending_interactions_for_run(state_db, agent_id, "cli_requested_stop")
                .await?;
        }
...
async fn delete_agent(
    state_db: &StateRuntime,
    agent_id: &str,
) -> anyhow::Result<(bool, Option<BackgroundAgentRun>)> {
    let existing_run = state_db
        .get_background_agent_run(agent_id)
        .await
        .context("failed to read background agent before delete")?;
    let deleted = state_db
        .request_background_agent_delete(agent_id)
        .await
        .context("failed to request background agent delete")?;
    if deleted {
        let terminalize_immediately = existing_run.as_ref().is_some_and(|run| {
        ...
        if terminalize_immediately {
            cancel_active_pending_interactions_for_run(state_db, agent_id, "cli_requested_delete")
                .await?;
            upsert_lifecycle_status_snapshot(
                state_db,
                agent_id,
...
async fn cancel_active_pending_interactions_for_run(
    ...
    let interactions = state_db
        .list_background_agent_pending_interactions(run_id, None)
        .await
        .context("failed to list background agent pending interactions")?;
    for interaction in interactions {
        if is_active_pending_interaction(&interaction) {
            state_db
                ...
                .with_context(|| {
                    format!(
                        "failed to cancel background agent pending interaction {}",
                        interaction.id
                    )
                })?;
        }
...
async fn upsert_lifecycle_status_snapshot(
    ...
    let Some(run) = state_db
        ...
        .await
        .context("failed to read background agent after lifecycle update")?
    else {
        return Ok(());
    };
    let pending_interaction_count = active_pending_interaction_count(state_db, run_id).await?;
    state_db
        .upsert_background_agent_status_snapshot(&BackgroundAgentStatusSnapshotParams {
        ...
        .await
        .map(|_| ())
        .context("failed to update background agent lifecycle snapshot")
}

async fn active_pending_interaction_count(
    state_db: &StateRuntime,
    run_id: &str,
) -> anyhow::Result<i64> {
    let count = state_db
        ...
        .context("failed to list background agent pending interactions")?
        .into_iter()
        .filter(is_active_pending_interaction)
        .count();
    Ok(i64::try_from(count).unwrap_or(i64::MAX))
}

fn is_active_pending_interaction(interaction: &BackgroundAgentPendingInteraction) -> bool {
    matches!(
        interaction.status,
...
mod tests {
    ...
    use codex_state::BackgroundAgentRetentionState;
    use pretty_assertions::assert_eq;
    use serde_json::json;

    async fn create_cli_run(
        state_db: &StateRuntime,
        agent_id: &str,
        ...
        state_db
            .create_background_agent_run(&BackgroundAgentRunCreateParams {
                ...
                status_reason: Some("queued by test".to_string()),
                config_fingerprint: None,
                version_fingerprint: Some("test-version".to_string()),
            })
            .await
    }
        ...
        state_db
            .create_background_agent_pending_interaction(
                &BackgroundAgentPendingInteractionCreateParams {
                    ...
                    worker_request_id: Some(format!("worker-{interaction_id}")),
                    kind: BackgroundAgentPendingInteractionKind::UserInput,
                    request_payload_json: json!({"prompt": "need input"}),
    ...
    #[tokio::test]
    async fn stop_agent_terminalizes_unclaimed_run_and_snapshot() -> anyhow::Result<()> {
        let temp = tempfile::tempdir()?;
        let state_db =
            StateRuntime::init(temp.path().to_path_buf(), "test-provider".to_string()).await?;
        create_cli_run(state_db.as_ref(), "stop-run").await?;
        create_pending_interaction(state_db.as_ref(), "stop-run", "stop-pending").await?;

        let run = stop_agent(state_db.as_ref(), "stop-run")
            .await?
        ...
        let interaction = state_db
            .get_background_agent_pending_interaction("stop-pending")
            .await?
        ...
        let snapshot = state_db
            .get_background_agent_status_snapshot("stop-run")
            .await?
            .expect("snapshot should exist");
        assert_eq!(snapshot.status, BackgroundAgentRunStatus::Cancelled);
        assert_eq!(snapshot.desired_state, BackgroundAgentDesiredState::Stopped);
    ...
    #[tokio::test]
    async fn delete_agent_terminalizes_unclaimed_run_and_snapshot() -> anyhow::Result<()> {
        let temp = tempfile::tempdir()?;
        let state_db =
            StateRuntime::init(temp.path().to_path_buf(), "test-provider".to_string()).await?;
        create_cli_run(state_db.as_ref(), "delete-run").await?;
        create_pending_interaction(state_db.as_ref(), "delete-run", "delete-pending").await?;

        let (deleted, run) = delete_agent(state_db.as_ref(), "delete-run").await?;
        let run = run.expect("run should exist");
        ...
        assert_eq!(
            run.retention_state,
            BackgroundAgentRetentionState::DeleteRequested
        );
        let interaction = state_db
            .get_background_agent_pending_interaction("delete-pending")
            .await?
            .expect("pending interaction should exist");
        assert_eq!(
            interaction.status,
        ...
        let snapshot = state_db
            .get_background_agent_status_snapshot("delete-run")
            .await?
            .expect("snapshot should exist");
        assert_eq!(snapshot.status, BackgroundAgentRunStatus::Cancelled);
        assert_eq!(snapshot.desired_state, BackgroundAgentDesiredState::Deleted);
        assert_eq!(snapshot.summary.as_deref(), Some("Deleted"));
        assert_eq!(snapshot.pending_interaction_count, 0);
        assert_eq!(snapshot.last_event_seq, run.last_event_seq);
