...
impl StateRuntime {
    ...
    pub async fn upsert_background_agent_status_snapshot_for_supervisor(
        ...
        let now = Utc::now().timestamp();
        let mut tx = self.pool.begin().await?;
        if !guard_background_agent_supervisor_write(
            &mut tx,
            params.snapshot.run_id.as_str(),
        ...
        upsert_background_agent_status_snapshot_in_tx(&mut tx, &params.snapshot, now).await?;
        tx.commit().await?;

        self.get_background_agent_status_snapshot(params.snapshot.run_id.as_str())
            .await
    }
...
WHERE run_id = ?
    ...
    pub async fn create_background_agent_execution_snapshot(
        ...
        let now = Utc::now().timestamp();
        let mut tx = self.pool.begin().await?;
        let id = create_background_agent_execution_snapshot_in_tx(&mut tx, params, now).await?;
        tx.commit().await?;
    ...
    pub async fn create_background_agent_execution_snapshot_for_supervisor(
        ...
        let id = create_background_agent_execution_snapshot_in_tx(&mut tx, &params.snapshot, now)
            .await?;
        tx.commit().await?;

        self.get_background_agent_execution_snapshot(id).await
    }
...
async fn guard_background_agent_supervisor_write(
    tx: &mut sqlx::Transaction<'_, Sqlite>,
    run_id: &str,
    supervisor_id: &str,
    generation: i64,
...
async fn create_background_agent_execution_snapshot_in_tx(
    tx: &mut sqlx::Transaction<'_, Sqlite>,
    params: &BackgroundAgentExecutionSnapshotParams,
    now: i64,
) -> anyhow::Result<i64> {
...
WHERE run_id = ?
    ...
    .last_insert_rowid();
    let seq: i64 =
        sqlx::query_scalar("SELECT seq FROM background_agent_execution_snapshots WHERE id = ?")
            .bind(id)
            .fetch_one(&mut **tx)
            .await?;
