...
WHERE
    ...
    AND status IN ('starting', 'running', 'waiting_on_approval', 'waiting_on_user')
        ...
        if let Some(worker_request_id) = params.worker_request_id.as_deref()
            && let Some(existing) =
                get_background_agent_pending_interaction_by_worker_request_id_in_tx(
                    &mut tx,
...
async fn get_background_agent_pending_interaction_by_worker_request_id_in_tx(
...
SELECT
    ...
    updated_at
FROM background_agent_pending_interactions
WHERE run_id = ? AND worker_request_id = ?
            "#,
    )
    .bind(run_id)
