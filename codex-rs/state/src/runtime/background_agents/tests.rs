...
/// Generic guard for the background-agent state invariant: *no* persisted
/// column may hold a recoverable plaintext secret.
///
/// Per-column assertions have twice passed while a sibling column still
/// leaked, so this test asserts the invariant structurally instead: every
/// caller-supplied string on the admission request carries the same
/// secret-shaped token, and afterwards every cell of every `background_agent*`
/// table is scanned for that token. A newly added column therefore fails here
/// automatically unless it is digested, redacted, or provably non-secret.
#[tokio::test]
async fn background_agent_admission_persists_no_plaintext_secret_in_any_column()
-> anyhow::Result<()> {
    let runtime = StateRuntime::init(unique_temp_dir(), "test-provider".to_string()).await?;
    let secret = format!("{}{}", "sk-proj-", "W5wN".repeat(12));
    assert!(crate::local_state_string_contains_secret(&secret));
    let tainted = |field: &str| format!("inline:{secret}:{field}");

    let mut params = admission_params(
        "plaintext-sweep-run",
        tainted("idempotency-key").as_str(),
        tainted("auth-profile").as_str(),
    );
    // `id` is the one string the caller does not choose: it is generated by the
    // runtime and is the primary/foreign key joining every background-agent
    // table, so it must stay verbatim. Everything else is caller-controlled and
    // is therefore tainted here.
    params.request_id = Some(tainted("request-id"));
    params.source = tainted("source");
    params.prompt_snapshot_ref = tainted("prompt-snapshot-ref");
    params.input_snapshot_ref = Some(tainted("input-snapshot-ref"));
    params.thread_id = Some(tainted("thread-id"));
    params.thread_store_kind = tainted("thread-store-kind");
    params.thread_store_id = Some(tainted("thread-store-id"));
    params.rollout_path = Some(tainted("rollout-path"));
    params.parent_thread_id = Some(tainted("parent-thread-id"));
    params.parent_agent_run_id = Some(tainted("parent-agent-run-id"));
    params.spawn_linkage_json = Some(json!({"agentPath": [tainted("spawn-linkage")]}));
    params.status_reason = Some(tainted("status-reason"));
    params.config_fingerprint = Some(tainted("config-fingerprint"));
    params.version_fingerprint = Some(tainted("version-fingerprint"));

    let mut snapshot_params = admission_snapshot_params(&params);
    snapshot_params.recovery_policy = tainted("recovery-policy");
    let start_payload = admission_start_payload(&params);
    let (run, created, _, _, _) = runtime
        .admit_background_agent_run(
            &params,
            &start_payload,
    ...
    let tables = sqlx::query_scalar::<_, String>(
        "SELECT name FROM sqlite_master \
         WHERE type = 'table' AND name LIKE 'background\\_agent%' ESCAPE '\\' \
         ORDER BY name",
    )
    ...
    assert!(
        tables.len() >= 5,
        "expected the background-agent schema to be present, found {tables:?}"
    );
    ...
    for table in &tables {
        ...
        for column in &columns {
            ...
            // names come from the schema (never from input), so leaking the
            // handful of generated scan statements is safe inside this test.
            let scan_sql: &'static str = Box::leak(
                format!(
            ...
            let hits = sqlx::query_scalar::<_, i64>(scan_sql)
                .bind(secret.clone())
                .fetch_one(&mut *conn)
                .await?;
            if hits > 0 {
                leaks.push(format!("{table}.{column} ({hits} row(s))"));
            }
        }
    }
    drop(conn);
    assert!(scanned_columns > 0, "no background-agent columns scanned");
    assert!(
        leaks.is_empty(),
        "recoverable plaintext secret persisted in: {}",
        leaks.join(", ")
    );

    // The invariant must hold without disabling the feature: the run is still
    // readable and its refs still round-trip (redacted, not dropped).
    let stored = runtime
        .get_background_agent_run(run.id.as_str())
        .await?
        .expect("admitted run should be readable");
    assert_eq!(
        stored.prompt_snapshot_ref,
        crate::redact_local_state_string(tainted("prompt-snapshot-ref"))
    );
    assert!(!crate::local_state_string_contains_secret(
        &stored.prompt_snapshot_ref
    ));
    // Idempotent replay still resolves the same run from the raw key.
    let (replayed, replayed_created, _, _, _) = runtime
        .admit_background_agent_run(
            &params,
            &start_payload,
            &snapshot_params,
