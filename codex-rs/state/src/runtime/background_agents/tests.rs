...
#[tokio::test]
async fn background_agent_run_create_is_idempotent_by_request_id() -> anyhow::Result<()> {
    let runtime = StateRuntime::init(unique_temp_dir(), "test-provider".to_string()).await?;
    let first = create_run(runtime.as_ref()).await?;
    let second = runtime
        .create_background_agent_run(&BackgroundAgentRunCreateParams {
            id: "run-duplicate-request".to_string(),
            idempotency_key: None,
            request_id: Some("req-1".to_string()),
            source: "cli".to_string(),
            prompt_snapshot_ref: "prompt://duplicate-request".to_string(),
            input_snapshot_ref: None,
            thread_id: None,
            thread_store_kind: "local".to_string(),
            thread_store_id: None,
...
async fn concurrent_background_agent_events_get_unique_monotonic_sequences() -> anyhow::Result<()> {
    let runtime = StateRuntime::init(unique_temp_dir(), "test-provider".to_string()).await?;
    create_run(runtime.as_ref()).await?;
    ...
    for idx in 0..32 {
        ...
        tasks.push(tokio::spawn(async move {
            let payload = json!({ "idx": idx });
            runtime
                .append_background_agent_event("run-1", "agent.progress", &payload)
                .await
        }));
    }
    ...
    let run = runtime
        ...
        .await?
        .expect("run should exist");
    assert_eq!(run.last_event_seq, 32);
    Ok(())
}
...
-> anyhow::Result<()> {
    ...
    assert!(
        runtime
            .append_background_agent_event_for_supervisor(BackgroundAgentEventForSupervisorParams {
                run_id: "run-1",
                supervisor_id: "supervisor-1",
                generation: first_generation,
                write_scope: BackgroundAgentWorkerWriteScope::Active,
                event_type: "agent.staleEvent",
                payload_json: &json!({"stale": true}),
            })
            .await?
            .is_none()
    );
    assert!(
        runtime
            .create_background_agent_execution_snapshot_for_supervisor(
                BackgroundAgentExecutionSnapshotForSupervisorParams {
                    supervisor_id: "supervisor-1",
                    generation: first_generation,
                    snapshot: BackgroundAgentExecutionSnapshotParams {
                        run_id: "run-1".to_string(),
                        snapshot_kind: "stale_worker_snapshot".to_string(),
                        payload_json: json!({"stale": true}),
                        recovery_policy: "resume_or_orphan".to_string(),
                        config_fingerprint: None,
                    },
...
#[tokio::test]
async fn pending_interaction_replay_reuses_worker_request_id_after_reclaim() -> anyhow::Result<()> {
    let runtime = StateRuntime::init(unique_temp_dir(), "test-provider".to_string()).await?;
    create_run(runtime.as_ref()).await?;
    ...
    let first = runtime
        .create_background_agent_pending_interaction_for_supervisor(
            &BackgroundAgentPendingInteractionCreateParams {
                id: "interaction-original".to_string(),
                run_id: "run-1".to_string(),
        ...
        .await?
        .expect("interaction should be created");
    assert_eq!(first.id, "interaction-original");
    ...
    let replayed = runtime
        .create_background_agent_pending_interaction_for_supervisor(
            &BackgroundAgentPendingInteractionCreateParams {
                id: "interaction-replayed".to_string(),
                run_id: "run-1".to_string(),
                worker_request_id: Some("worker-request-1".to_string()),
                kind: BackgroundAgentPendingInteractionKind::Approval,
                request_payload_json: json!({"command": "true"}),
        ...
        .await?
        .expect("existing interaction should be reused");
