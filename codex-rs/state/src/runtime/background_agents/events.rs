...
WHERE run_id = ?
    ...
    .await?
    .last_insert_rowid();
    let seq: i64 = sqlx::query_scalar("SELECT seq FROM background_agent_events WHERE id = ?")
        .bind(id)
        .fetch_one(&mut **tx)
        .await?;
...
WHERE
    id =
        ...
        query.push_bind(params.run_id);
        query.push(" AND supervisor_id = ");
        query.push_bind(params.supervisor_id);
        query.push(" AND generation = ");
        query.push_bind(params.generation);
        match params.write_scope {
 ...
 AND desired_state =
                ...
                query.push_bind(BackgroundAgentDesiredState::Running.as_str());
                query.push(" AND retention_state = ");
                query.push_bind(crate::BackgroundAgentRetentionState::Active.as_str());
                query.push(
        ...
        let id = append_background_agent_event_in_tx(
            &mut tx,
            params.run_id,
            params.event_type,
            params.payload_json,
        ...
        self.get_background_agent_event(id).await
    }
