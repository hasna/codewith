...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    #[tokio::test]
    async fn rejects_case_aliased_apfs_worktree_paths_during_admission() -> anyhow::Result<()> {
        let runtime = test_runtime().await;
        let base_repo_path = test_temp_dir()?.join("repo");
        let canonical_path = base_repo_path
            .join(".codewith")
            .join("worktrees")
            .join("RunA");
        let case_alias = base_repo_path
            .join(".codewith")
            .join("worktrees")
            .join("runa");
        std::fs::create_dir_all(&base_repo_path)?;
        ...
        assert_ne!(
            ...
            path_to_db_string(&case_alias)
        );
        assert_eq!(
            managed_worktree_path_key_from_display(path_to_db_string(&canonical_path).as_str()),
            managed_worktree_path_key_from_display(path_to_db_string(&case_alias).as_str())
        );
        let store = runtime.managed_worktrees();
        store
            .create_managed_worktree(create_params_for_paths(
        ...
        let error = store
            ...
            .await
            .expect_err("case-insensitive APFS aliases must not bypass worktree admission");
        assert!(
