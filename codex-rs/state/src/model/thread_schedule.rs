...
impl ThreadScheduleRunRow {
    pub(crate) fn try_from_row(row: &SqliteRow) -> Result<Self> {
        Ok(Self {
            ...
            materialized_at_ms: row.try_get("materialized_at_ms")?,
            turn_input: row.try_get("turn_input")?,
...
impl TryFrom<ThreadScheduleRunRow> for ThreadScheduleRun {
    ...
    fn try_from(row: ThreadScheduleRunRow) -> Result<Self> {
        Ok(Self {
            ...
            lease_id: row.lease_id,
            turn_id: row.turn_id,
            materialized_at: optional_epoch_millis_to_datetime(row.materialized_at_ms)?,
            turn_input: row.turn_input,
            goal_id: row.goal_id,
            error: row.error,
