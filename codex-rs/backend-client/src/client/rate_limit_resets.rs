//! Backend client operations for reading and consuming rate-limit reset credits.
...
impl Client {
    pub async fn get_rate_limits_with_reset_credits(&self) -> Result<RateLimitsWithResetCredits> {
        let payload = self.get_rate_limit_status_with_reset_credits().await?;
    ...
    pub(super) async fn get_rate_limit_status_with_reset_credits(
        &self,
