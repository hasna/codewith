...
pub enum Command {
    /// Submit a new Codewith Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Codewith Cloud task.
    Status(StatusCommand),
    /// List Codewith Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Codewith Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Codewith Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Codewith Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `codewith cloud` to browse).
    ...
    /// Git branch to run in Codewith Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Codewith Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Codewith Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Codewith Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
